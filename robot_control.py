...
class RobotDrive:
    ...
    def pingSensors(self, raw_cmd="p", try_again=True, reset=True):
    ...
    def sendCommand(self, raw_cmd, timeout=0):
        ...
        while time.time() - start_time < timeout:
            ...
            if responses[0] == "+":
                break
        if self.verboseConsole:
            print(Fore.GREEN + f"Response for [{raw_cmd}] at {time_rx}: {responses}")
        return responses
    ...
    def changeSpeeds(self, motor1=0, motor2=0, motor3=0, motor4=0):
        for i, speed in enumerate([motor1, motor2, motor3, motor4]):
            if speed != 0:
                self.sendCommand(f"m{i+1}{speed}")  # change motor speed
        packet_tx = self.packetize("c")
        if packet_tx:
            self.transmit(packet_tx)
        start_time = time.time()
        while time.time() - start_time < self.RESPONSE_TIMEOUT:
            [responses, _] = self.receive()
            if responses[0] == "c":
                continue
            if responses[0] != "+" and responses[0] is not False:
                break
    ...
    def simpleParallelize(self, ping=True, check_for_walls=True):
        if ping:
            self.pingSensors()
    ...
    def pingLoadSensors(self, try_again=True, reset=True):
    ...
    def detectLoad(self):
        ...
        servo1, servo1_open, servo1_close = 1, 0, 180
    ...
    def loadingZoneSequence(self, didOnce=False, doOnlyOnce=False):
        while (not self.holdingLoad() or not didOnce) or (
            ...
            self.detectLoad()
            didOnce = True
        self.simpleParallelize(check_for_walls=False)
        self.getToWall()
        self.simpleParallelize()
        self.performBlockCentering()
