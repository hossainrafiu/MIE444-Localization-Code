...
class ParticleFilter:
    ...
    def move_particles_improved(self, delta_x, delta_y, delta_theta):
        ...
        for p in self.particles:
            # Add noise proportional to motion magnitude
            motion_magnitude = np.sqrt(delta_x**2 + delta_y**2)
            noise_scale = max(0.1, motion_magnitude * 0.1)  # Adaptive noise
    ...
    def update_weights_improved(self, sensor_readings):
        ...
        if total_weight < 1e-50:  # Very small threshold
        ...
        # Normalize weights
        for i, p in enumerate(self.particles):
            self.particles[i] = p._replace(weight=p.weight / total_weight)
    ...
    def resample_particles(self):
        ...
        print("sum at resample:", sum(weights))
        if sum(weights) > 2:
    ...
    def estimate_position(self):
        ...
        theta = sum(p.theta * p.weight for p in self.particles)
        return (x, y, theta)
...
if __name__ == "__main__":
    ...
    for motion_noise in [1.0]:
        ...
        for move in movements:
            ...
            print(
                f"Estimated Position: {estimated_pos}, Actual Position: {actual_position}"
            )
